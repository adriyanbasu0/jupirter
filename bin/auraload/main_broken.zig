...
pub fn main() !void {
    ...
    trampoline(@as(*anyopaque, @ptrFromInt(entry_point)), @as(*anyopaque, @ptrFromInt(aligned_stack)), &trampoline_alloc, &trampoline_free);
        data_addr = posix.mmap(null, header.data_size + page_size, posix.PROT.READ | posix.PROT.WRITE, .{ .TYPE = .PRIVATE, .ANONYMOUS = true }, -1, 0) catch {
            try out.write("Error: Cannot map data section\n");
            try out.flush();
            std.debug.print("Error: Cannot map data section\n", .{});
            posix.munmap(code_slice);
            return error.MmapFailed;
        ...
        try out.print("Copying {d} bytes of data\n", .{header.data_size});
        try out.flush();
        @memcpy(data_addr.?[0..header.data_size], @as([*]u8, @ptrFromInt(aligned_data))[0..header.data_size]);
        try out.write("Data copy complete\n");
        try out.flush();
    }
    ...
    try out.write("Memory mapped successfully\n");
    try out.print("Code slice: 0x{x} - 0x{x}\n", .{ @intFromPtr(code_slice.ptr), @intFromPtr(code_slice.ptr) + code_slice.len });
    try out.print("Aligned text: 0x{x}\n", .{aligned_text});
    try out.print("Aligned data: 0x{x}\n", .{aligned_data});
    try out.print("Aligned stack: 0x{x}\n", .{aligned_stack});
    try out.flush();

    const entry_point = aligned_text + header.entry_point;
    try out.write("\n=== Executing Program ===\n");
    try out.print("Entry point: 0x{x} (aligned_text: 0x{x} + header.entry_point: 0x{x})\n", .{ entry_point, aligned_text, header.entry_point });
    try out.flush();

    // Check if the entry point is valid
    const entry_ptr = @as(*anyopaque, @ptrFromInt(entry_point));
    try out.print("About to call trampoline with entry_ptr: 0x{x}\n", .{@intFromPtr(entry_ptr)});
    try out.flush();

    trampoline(entry_ptr, @as(*anyopaque, @ptrFromInt(aligned_stack)), &trampoline_alloc, &trampoline_free);

    try out.write("Trampoline returned successfully\n");
    try out.flush();
}
